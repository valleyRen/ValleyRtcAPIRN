...
#pragma clang diagnostic pop
    ...
    if (method != nil) {
        NSDictionary* out = [self performSelector:method withObject:(objBase*)event withObject:index];
        NSError *error = nil;
        NSData *jsonData = [NSJSONSerialization dataWithJSONObject:out options:NSJSONWritingPrettyPrinted error:&error];
        if (error) {
            RCTLogInfo(@"==== error code = %ld, domain = %@", (long)error.code, error.domain);
        }
...
RCT_EXPORT_METHOD(ChannelDisableAudio:(NSInteger)index callback:(RCTResponseSenderBlock)callback) {
    RCTLogInfo(@"=======  ChannelDisableAudio, index = %ld", (long)index);
    IRtcChannel *rtcChannel = [rtcChannelDic objectForKey:@(index)];
    int ec = ERR_NOT_INITIALIZE;
    if (rtcChannel) {
