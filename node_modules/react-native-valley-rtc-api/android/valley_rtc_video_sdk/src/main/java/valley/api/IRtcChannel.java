...
public class IRtcChannel {
    ...
    public final static  int IID_AUDIOSYSTEM    = 0x04;
  ...
  //  #define NO_ROTATE_TYPE_90               ( 1 )
    ...
    public int SetSpeakerOn(int On) {  // 扬声器和听筒切换 -1 耳机互动， 0 听筒 1 扬声器 建议在-1和0之间切换
        if(null != mAudioChannel){
            com.rtc.client.IRtcAudioSystem system = (com.rtc.client.IRtcAudioSystem)mAudioChannel.GetInterface(IID_AUDIOSYSTEM);
            if(null == system)
                return ERR_NOT_INITIALIZE;
            system.SetSpeakerphoneOn(On);
            return ERR_SUCCEED;
        }
        else{
            return ERR_UNSUPPORT;
